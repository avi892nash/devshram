...
export default function QuoteCard({
  ...
  return (
    <div className={`mx-auto font-mono ${className}`}>
      {/* Main Quote Box */}
      <div className="relative bg-lightBackground text-white px-6 py-4 border border-white/20">
        <QuotationMark className="absolute -top-3 left-3" />

        <p className="text-base md:text-lg whitespace-pre-wrap leading-relaxed">
          {quote}
        </p>

        {/* Closing Quote near author box */}
        <QuotationMark className="absolute -bottom-3 right-3 rotate-180" />
        
      </div>
      {/* Author Block with cut edge */}
      <div className="flex justify-end">
        <div className="bg-lightBackground text-white text-base px-4 py-2 border border-white/20 ">
          - {author}
        </div>
      </div>
    </div>
